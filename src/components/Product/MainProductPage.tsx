                        ...
                        <div className='flex space-x-3'>
                            {products?.product?.availableStorages?.map((storage: any, index: number) => (
                                <div key={index} className="flex items-center justify-center text-sm w-fit px-2 py-1 h-fit border-2 border-black">
                                    <p className="font-bold">{storage?.value}</p>
                                </div>
                            ))}
                        </div>
                        <div className='flex space-x-3'>
                            ...
                            <tbody>
                                {products?.product?.vendors?.map((shop: any, index: number) => (
                                    <tr key={index + 12}>
                                                ...
                                                <>
                                                    ...
                                                    {price?.colors?.length <= 0 && (
                                                        <td className="text-[#353535] flex item-start m-auto p-2">
                                                            {price?.colors.map((color: any, colorIndex: number) => (
                                                                <div key={colorIndex} style={{
                                                                    backgroundColor: `${color}`,
                                                                }} className={` bg-[#${color}] rounded-full h-4 flex w-4 m-1`}></div>
                                                            ))}
                                                        </td>
                                                    )}
                                                </>
                                        ...
                                        {products?.product?.availableStorages?.map((storage: any, storageIndex: number) => (
                                            <td key={storageIndex} className="text-[#353535] item-start m-auto p-2">
                                                {storage.value ? storage.value : 'N/A'}
                                            </td>
                                        ))}
                                    </tr>
                                ))}
