...
import { getKomparasCodebyCode, updateIsShopSoldConfirmToTrue } from "../api/shops";
...
const ConfirmShop = () => {
  ...
  const handleConfirmToTrue = async () => {
    try {
      const res = await updateIsShopSoldConfirmToTrue(KomparasId);
        ...
        {data !== null && (
          <div className="flex flex-col py-12 w-fit min-h-96 h-fit border-gray-950 border-2 m-auto justify-start gap-4 p-3 items-start">
            ...
            <form className="flex gap-4 mt-10 w-full">
              {data?.shop_sold_confirm ? (
                <div className="flex relative gap-4 w-full mt-8">
                  <p className="text-green-600 text-lg">
                    Wamaze kwemeza ko wagurishije iyi telephone
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <label
                    ...
                    htmlFor="isBuyer"
                  >
                    Nda shaka kwemeza ko nagurishie telephone
                  </label>
                </>
              )}
            </form>
